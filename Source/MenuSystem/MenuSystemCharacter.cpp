...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	if (!OnlineSessionInterface.IsValid()) //ensure is valid
	{
		return;
	}

	auto ExistingSession = OnlineSessionInterface->GetNamedSession(NAME_GameSession); //get existing session
	if (ExistingSession != nullptr)
	{
		OnlineSessionInterface->DestroySession(NAME_GameSession); //destroy session so that we can make a new one
	}
